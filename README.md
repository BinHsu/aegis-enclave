...
| **2.5.1** | ✅ done | Phase 2.5 polish (V2 promotion batch — pre-Wed final-window prep) | P0: cloud-evidence dimension fixes (5/6 empty PNGs root-caused — RDS identifier / ElastiCache Serverless `CacheName` / ALB `arn_suffix`), 7-step cloud-smoke with cache-hit/miss + partial-overlap demo (was 6-step measuring nothing), `ruff format --check` in `make lint`, RDS engine-version pre-flight in `ts_apply.sh` (deprecated 16.3 gotcha closed). P1: terraform community modules pinned to exact patch (case-study reproducibility), `uv.lock` committed (87 packages, 1050 sha256 hashes), DLQ pattern = alarm + manual triage script (ADR-0038 — auto-retry worker is anti-pattern). P2: ADR-0037 secrets-rotation deferred (manual procedure documented), deployment_guide § Production hardening checklist (8 forker-add items: rotation, ALB cert, bootstrap driver, observability, multi-region, VPC Flow Logs, Dependabot, Budgets/Anomaly), M4 arm64 native build verified. Cost-transparency reframe: per-hour table on README + honest FinOps scope (only `default_tags` + cost estimate, NO budget cap). Forker UX quick wins (`.leakguard.example` template, README clarifies tf-bootstrap is prod-only, smoke health-probe gives 4-step VPN diagnosis, cloud-up pre-flights `easyrsa`). |
