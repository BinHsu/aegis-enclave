...
- **Wed evidence**: dashboard screenshot (live during the apply window), alarm state strip showing OK across all 7 alarms, optionally a deliberate-trigger test (Bin sets one alarm to ALARM via CLI → email arrives → screenshot the email + EventBridge log entry of the state transition).
- **Reviewer signal**: PDF screenshots show p99 latency line + 500ms SLO threshold annotation, error rate line + 1.44%/0.6%/0.1% threshold annotations (multi-window burn rate visually present), cache hit ratio line + 80% threshold. buyer-grade reviewer reads "metered SLO with multi-window burn-rate alarms" — the same signal Grafana SLO plugin would carry, in CloudWatch's UI.
