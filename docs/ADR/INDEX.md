...
| [0051](0051-ci-driven-iam-apply-gh-tf-apply-enclave.md) | CI-driven IAM apply — `gh-tf-apply-enclave` OIDC role (named in the `gh-tf-*` family so the org SCP glob covers its IAM mutations) + apply-on-main / destroy-on-dispatch workflows; the human `make cloud-up/down` cannot create or tear down IAM in the governed org. Forker keeps the end-to-end local path. Supersedes CLAUDE.md §4 + ADR-0015 for the governed path only. |
