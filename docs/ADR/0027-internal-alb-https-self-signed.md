...
The operator's curl pattern from macOS during the Phase 2.3 acceptance window:
...
| **Sidecar TLS terminator (e.g., Envoy / nginx as a Fargate sidecar in front of the app)** | Decouples TLS from ALB, gives more flexibility (mTLS, custom headers, etc.). Adds a sidecar container, increases cognitive surface, blocks direct `terraform-aws-modules/alb` use. Over-engineered for the case-study scope and slows the Phase 2.3 acceptance window. Production architectures with strict mTLS requirements would revisit this. |

## Consequences

- The Phase 2.3 evidence section in `docs/deployment_guide.md` uses `https://` curl invocations consistently with the rest of the documentation, no `http://` mismatch to explain.
- The operator's curl command is more verbose (`--cacert` + `--resolve`) than the HTTP equivalent — strategy.md Phase 2.3 sequence pre-bakes a `CURL=` shell alias to compress the flags into a single repeated variable.
...
- One-year cert validity means a sustained deployment crossing the one-year boundary needs a re-apply or pre-emptive re-issue. Phase 2.3 is a 3-hour window, so this is theoretical for the case-study cycle but worth flagging for any operator extending the deliverable into a long-lived deployment.
...
- [ADR-0015](0015-no-k8s-no-real-apply.md) — supersession block; Phase 2.3 enabled by this ADR also enables the cloud-acceptance window that makes ALB HTTPS visible
