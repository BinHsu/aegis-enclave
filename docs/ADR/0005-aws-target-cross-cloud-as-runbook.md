...
## Context
The case-study brief lists "AWS or IONOS Cloud" as recommended-but-not-mandatory cloud targets. The deliverable must include cloud-side architecture and migration story, but the 15-hour budget (ADR-0002) does not stretch to a credible production deployment on a cloud the candidate has no operational receipts for.
