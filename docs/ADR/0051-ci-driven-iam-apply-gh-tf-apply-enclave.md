...
- **apply** — **manual `workflow_dispatch`** (a cost-posture refinement of the original push-to-main intent: the enclave stack carries real hourly cost — ECS Fargate / Client VPN / ElastiCache — so deploys are deliberate, unlike the always-cheap landing-zone fabric the sibling push-applies). Assumes `gh-tf-apply-enclave`, runs the IAM-creating apply. Remote state is an S3 backend the workflow writes as a **gitignored `backend.tf` at init time** (from the `AWS_TF_STATE_BUCKET` repo var) so `main.tf` stays backend-less and a forker's local `make cloud-up` (-backend=false) is unaffected. Deploy tfvars (cert ARNs, IPAM pool ids per ADR-0050, route53) come from the `AWS_DEPLOY_TFVARS` secret.
