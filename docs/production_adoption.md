...
The deliverable is calibrated to ADR-0003 (PoC scope, prod hygiene). When the workload outgrows that — many services, multi-team service mesh, autoscaling beyond Fargate's per-task ceiling, multi-region active-active — the upgrade paths live in the Phase 2 runbooks. `docs/migration_runbook.md` Track 3 covers ECS → EKS; `docs/scaling_runbook.md` covers single-region → multi-region. The composition is a starting point, not an end state.
