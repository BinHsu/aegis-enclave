...
- VPC quotas sufficient per region: 1 VPC + 3 private subnets + 1 ALB + 1 ECS cluster + 1 ElastiCache Serverless cache + 1 Client VPN endpoint + 1 S3 result bucket (independent per region — no cross-region replication; cross-region availability is provided by recompute-on-miss, see [ADR-0049](ADR/0049-cross-region-result-recompute-on-miss.md) which supersedes the CRR half of [ADR-0048](ADR/0048-large-result-store-s3-cross-region-replication.md))
...
- **`make dr-acceptance`** (multi-region only) is the one-button end-to-end DR proof: `cloud-up` both regions → prompt for VPN → **VERIFY #1** cross-region recompute (the `cross-region-check` above) → **kill** the peer region (`terraform destroy -target`) → **VERIFY #2** the survivor still serves a fresh `POST → poll → done` job → `cloud-down`. Proves both "cross-region data is available" (ADR-0049) and "losing a region does not take the service down" (ADR-0008 / ADR-0042) in a single run. Confirmation gates guard the destructive steps; `--keep-up` stops before the final teardown.
- **`make dr-drill`** is the narrower companion — destroy + **rebuild** the peer region and measure recovery **RTO** while a survivor probe runs, writing `logs/dr-report-<UTC>.md`. Use `dr-acceptance` to prove the DR contract end to end; use `dr-drill` to time a region rebuild.
