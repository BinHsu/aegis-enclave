...
- AWS regions selected (defaults `primary_region = eu-central-1`, `secondary_region = eu-west-1` — see `terraform/variables.tf`)
- VPC quotas sufficient per region: 1 VPC + 3 private subnets + 1 ALB + 1 ECS cluster + 1 ElastiCache Serverless cache + 1 Client VPN endpoint
