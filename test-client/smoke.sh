...
#
# Runs INSIDE the test-client container, which sits on the `edge` Docker
# network ONLY — it has NO direct path to `app` / `worker` / data tier (those
# live on the `internal: true` network). The only way it can reach `app:8000`
# is to bring up the WireGuard tunnel and let the dual-homed gateway forward
# into internal. This is the routed-VPN model (mirrors AWS Client VPN -> VPC
# private ALB) and is the property we verify in step 0.
#
# Seven steps + 4a/4b compute-path sub-coverage (#13 added step 0 routed-VPN
# verification; #9 item 1 added 4a/4b — without them the demo only exercises
# the cache path and never the prime compute kernel):
#   0.  Bring up WireGuard tunnel; verify gateway reachable + CoreDNS resolves app
#   1.  POST /primes → 202 + execution_id          (through tunnel)
...
# Packages: curl/jq for HTTP; python3+py3-sympy for the prime oracle (avoids PEP 668);
# wireguard-tools+openresolv+iproute2+iptables for `wg-quick up` to manage wg0.
apk add --no-cache curl jq python3 py3-sympy wireguard-tools openresolv iproute2 iptables >/dev/null 2>&1 || true
...
# ─── Step 0: Bring up WireGuard tunnel ───────────────────────────────────────
step "0/6  Bring up WireGuard tunnel (routed-VPN to internal)"

PEER_CONF=/wg-config/peer1/peer1.conf
# Gateway may still be initialising on first up; wait for peer1.conf.
i=0
while [ ! -f "$PEER_CONF" ] && [ "$i" -lt 30 ]; do
    sleep 1
    i=$((i + 1))
done
[ -f "$PEER_CONF" ] || fail "peer1.conf not generated by wg-gateway after 30s (image init not finished?)"

# Rewrite Endpoint from the linuxserver-auto-detected host public IP to the
# Docker network name — the UDP handshake then stays inside the `edge` Docker
# network (no host NAT hairpinning), which is the only place test-client can
# reach the gateway.
mkdir -p /etc/wireguard
sed 's|^Endpoint = .*|Endpoint = wg-gateway:51820|' "$PEER_CONF" > /etc/wireguard/wg0.conf
chmod 600 /etc/wireguard/wg0.conf

# Idempotent: tear down a stale tunnel left by a previous run on the long-lived
# test-client container before bringing the fresh one up.
wg-quick down wg0 >/dev/null 2>&1 || true
wg-quick up wg0 2>&1 | sed 's/^/  wg-quick: /'
trap 'wg-quick down wg0 >/dev/null 2>&1 || true' EXIT
wg show wg0 | sed 's/^/  /'

# Verify the gateway is pingable through the tunnel (proves routing works).
# (Some Alpine `ping` builds may need `iputils-ping`; fall back to a curl
# probe of the gateway's CoreDNS port if ping is unavailable.)
if command -v ping >/dev/null 2>&1; then
    ping -c 1 -W 3 10.13.13.1 >/dev/null 2>&1 || fail "gateway 10.13.13.1 unreachable through tunnel"
else
    # CoreDNS listens on :53 on the gateway; a TCP connect proves L3 reachability.
    nc -z -w 3 10.13.13.1 53 2>/dev/null || fail "gateway 10.13.13.1:53 unreachable through tunnel"
fi

# Verify CoreDNS on the gateway resolves an internal hostname — proves the
# DNS-via-tunnel path works (peer's /etc/resolv.conf was set to 10.13.13.1
# by wg-quick reading the `DNS = 10.13.13.1` line in peer1.conf).
APP_IP=$(getent hosts app 2>/dev/null | awk '{print $1}' | head -1)
[ -n "$APP_IP" ] || fail "DNS-via-gateway didn't resolve 'app' — check Corefile / openresolv"
case "$APP_IP" in
    172.30.*) ;;
    *) fail "app resolved to '$APP_IP' (expected 172.30.x.x); something is leaking past the tunnel" ;;
esac
ok "tunnel up; gateway pingable @10.13.13.1; app resolves to $APP_IP via gateway CoreDNS"
